...
from src.russian_preprocess import russian_preprocess_audio, mel_to_audio_rus
...
def predict_russian():
    try:
        ...
        # Step 1: Preprocess
        spectrogram, original_spectrogram = russian_preprocess_audio(file)
        spectrogram_tensor = torch.tensor(spectrogram).float().to(device='cpu')

        # Step 2: Predict
        russian_pred_model.eval()
        with torch.no_grad():
            pred_tensor = russian_pred_model(spectrogram_tensor).cpu().numpy()
        ...
        wav = mel_to_audio_rus(pred_spectrogram)
